...
{
	ostr << std::to_string(m.a);

	if ((m.xyzPow % 1000) / 100 != 0)
		ostr << "*x^" + std::to_string((m.xyzPow % 1000) / 100);

	if ((m.xyzPow % 100) / 10 != 0)
		ostr << "*y^" + std::to_string((m.xyzPow % 100) / 10);

	if (m.xyzPow % 10 != 0)
		ostr << "*z^" + std::to_string(m.xyzPow % 10);
...
std::ostream& operator << (std::ostream& ostr, const Polynom& p)
{
	if (p.monoms.begin() != p.monoms.end())
	{
		...
		{
			if ((*iter).GetSign() && iter != p.monoms.begin()) ostr << "+";
